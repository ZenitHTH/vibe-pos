...
export default function ImageManagePage() {
    ...
    const handleDelete = async (image: Image) => {
        ...
        if (usage.length > 0) {
            if (!confirm(`This image is used by ${usage.length} products. Deleting it will unlink it from them. Continue?`)) {
                return;
            }
        } else {
    ...
    const toggleLink = async (productId: number, isLinked: boolean) => {
        ...
        try {
            if (isLinked) {
                await invoke("unlink_product_image", { key: dbKey, productId, imageId: selectedImage.id });
                setLinks(prev => prev.filter(l => !(l.product_id === productId && l.image_id === selectedImage.id)));
            } else {
                await invoke("link_product_image", { key: dbKey, productId, imageId: selectedImage.id });
                setLinks(prev => [...prev, { product_id: productId, image_id: selectedImage.id }]);
            }
        } catch (err) {
    ...
    const getProductUsage = (imageId: number) => {
        const productIds = links.filter(l => l.image_id === imageId).map(l => l.product_id);
                        ...
                        <Card key={img.id} className="overflow-hidden group relative">
                            <div className="aspect-square bg-muted flex items-center justify-center relative">
                                ...
                                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                    <Button size="sm" variant="secondary" onClick={() => openLinkModal(img)} title="Link to Products">
                                        <FaLink />
                                    </Button>
                                    <Button size="sm" variant="destructive" onClick={() => handleDelete(img)} title="Delete">
                                        <FaTrash />
                                    </Button>
                                </div>
                            </div>
                            <div className="p-3 text-sm">
                                <div className="font-medium truncate" title={img.file_name}>{img.file_name}</div>
                                <div className="text-muted-foreground text-xs mt-1">
                                    {usedBy.length === 0 ? "Unused" : `Used by ${usedBy.length} products`}
                                </div>
                            </div>
                        </Card>
                    );
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="max-h-[300px] overflow-y-auto space-y-2 border rounded-md p-2">
                            {filteredProductsToLink.map(product => {
                                const isLinked = links.some(l => l.product_id === product.product_id && l.image_id === selectedImage.id);
                                return (
                                    <div key={product.product_id} className="flex items-center justify-between p-2 hover:bg-muted rounded-md">
